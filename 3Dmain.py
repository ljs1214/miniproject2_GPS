...
short_path = dict(nx.all_pairs_dijkstra_path_length(G))  # dijkstra搜路
new_sa = []
for i in range(len(sa)):
    temp = []
    for j in range(len(sa[i])):
        temp.append(short_path[a_point_name[j]][s_point_name[i]])
    new_sa.append(temp)
...
for i in range(len(new_sa)):
    ...
    for i in range(len(new_sa)):
        ...
        if u == 1 and s < adv_dis / 1.25:
            ...
            if bb != -1:
                ...
                predict_z = a_position[aa][2] + random.normalvariate(bb,0.01)
        else:
            predict_x = 0
            predict_y = 0
            predict_z = 0
    ...
    while True:
        r_a_hat = []
        for j in range(len(sa[i])):
            r_a_hat.append(((predict_x - a_position[j][0]) ** 2 + (predict_y - a_position[j][1]) ** 2
                            + (predict_z - a_position[j][2]) ** 2) ** (1 / 2))
        hp = []
        for j in range(len(sa[i])):
            temp = []
            temp.append(-((a_position[j][0]) - predict_x) / r_a_hat[j])
            temp.append(-((a_position[j][1]) - predict_y) / r_a_hat[j])
            temp.append(-((a_position[j][2]) - predict_y) / r_a_hat[j])
            hp.append(temp)
        hp = np.array(hp)
        r_to_list = []
        ...
        for j in range(len(sa[0])):
            if sa[i][j] != 0:
                dis += (((xy_list[0]-a_position[j][0])**2+(xy_list[1]-a_position[j][1])**2+
                         (xy_list[2]-a_position[j][2])**2)**0.5 - sa[i][j]) ** 2
                h += 1
        if h != 0:
            dis = dis / h
        else:
            ...
            predict_y = xy_list[1]
            predict_z = xy_list[2]
            ans_list.append([predict_x, predict_y,predict_z])
        ...
        if abs(dis) < epsilon1 or abs(dis-error)<epsilon2:
            ...
            error = dis
        echo += 1
plt.legend(["0.0005","0.01","0.02","0.04"])
plt.show()
print(ans_list)
print(len(ans_list))
...
for i in range(len(a_position)):
    ax1.scatter3D(a_position[i][0],a_position[i][1],color="red")
#plt.show()
for i in ans_list:
    ax1.scatter3D(i[0],i[1],i[2],color="blue")
plt.show()
